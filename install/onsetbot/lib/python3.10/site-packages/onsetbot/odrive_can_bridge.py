...
AXIS_STATE_IDLE = 1
CONTROLLER_MODE_TORQUE_CONTROL = 1
...
class ODriveCANBridge (Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("command_axis_ids", [1])
        ...
        self.declare_parameter("topic_axis_command_prefix", "/odrive_axis")
        ...
        self.topic_axis_command_prefix: str = str(
            self.get_parameter("topic_axis_command_prefix").value
        )
        self._enc_rr_idx = 0
        ...
        self._feedback_axis_ids: List[int] = list(self.axis_ids)
        if not self._feedback_axis_ids:
            self._feedback_axis_ids = list(self.axis_ids)

        # Deadman Timer
        self.last_command_time: Dict[int, float] = {
            axis_id: 0.0 for axis_id in self.axis_ids
        }
        self.last_command_vel: Dict[int, float] = {axis_id: 0.0 for axis_id in self.axis_ids}
        self.last_command_torque: Dict[int, float] = {axis_id: 0.0 for axis_id in self.axis_ids}
        ...
        self.axis_command_subscriptions = {
            axis_id: self.create_subscription(
                ControlMessage,
                f"{self.topic_axis_command_prefix}{axis_id}/control_message",
                lambda msg, axis_id=axis_id: self.axis_command_callback(msg, axis_id),
                10,
            )
    ...
    def _send_set_input_torque(self, axis_id: int, torque_nm: float):
        arb_id = make_arb_id(axis_id, CMD_SET_INPUT_TORQUE)
    ...
    def _apply_velocity_command(
        ...
        effective_control_mode = (
            self.command_control_mode if control_mode is None else int(control_mode)
        )
        effective_input_mode = (
            self.command_input_mode if input_mode is None else int(input_mode)
        )

        if (self.last_controller_mode.get(axis_id) != effective_control_mode or
                self.last_input_mode.get(axis_id) != effective_input_mode):
            self._set_controller_mode(
                axis_id,
                effective_control_mode,
                effective_input_mode,
            )
            self.last_controller_mode[axis_id] = effective_control_mode
            self.last_input_mode[axis_id] = effective_input_mode

        self.last_command_vel[axis_id] = velocity
        self.last_command_torque[axis_id] = 0.0
        self.last_command_time[axis_id] = time.time()
        self._send_set_input_vel(axis_id, velocity, 0.0)

    def _apply_torque_command(
        self,
        axis_id: int,
        torque: float,
        control_mode: Optional[int] = None,
        ...
        self.last_command_torque[axis_id] = torque
        self.last_command_time[axis_id] = time.time()
        self._send_set_input_torque(axis_id, torque)

    def command_callback(self, msg: ControlMessage):
        """Handle shared velocity commands and fan out to configured target axes."""
        if self._shutdown_requested:
            return

        axis_ids = self.command_axis_ids if self.command_axis_ids else list(self.axis_ids)
        if not axis_ids:
            return
        ...
        if msg.control_mode == CONTROLLER_MODE_VELOCITY_CONTROL:
            ...
            for idx, axis_id in enumerate(axis_ids):
                ...
                self._apply_velocity_command(axis_id, vel, msg.control_mode, msg.input_mode)
            return

        if msg.control_mode == CONTROLLER_MODE_TORQUE_CONTROL:
            base_torque = float(msg.input_torque)
            for idx, axis_id in enumerate(axis_ids):
                torque = -base_torque if (self.command_mirror and (idx % 2 == 1)) else base_torque
                self._apply_torque_command(axis_id, torque, msg.control_mode, msg.input_mode)
            return

        self.get_logger().warn(f"Unsupported ControlMessage control_mode={msg.control_mode}; ignoring")

    def axis_command_callback(self, msg: ControlMessage, axis_id: int):
        """Handle a direct per-axis velocity command without mirrored fan-out."""
        if self._shutdown_requested:
            return

        if msg.control_mode == CONTROLLER_MODE_VELOCITY_CONTROL:
            self._apply_velocity_command(axis_id, float(msg.input_vel), msg.control_mode, msg.input_mode)
            return

        if msg.control_mode == CONTROLLER_MODE_TORQUE_CONTROL:
            self._apply_torque_command(axis_id, float(msg.input_torque), msg.control_mode, msg.input_mode)
            return
    ...
    def _command_watchdog(self):
        ...
        for aid in self.axis_ids:
            last_time = self.last_command_time.get(aid, 0.0)
            if last_time <= 0.0:
                continue
            if (now - last_time) > self.command_timeout_sec:
                if self.last_controller_mode.get(aid) == CONTROLLER_MODE_TORQUE_CONTROL:
                    if self.last_command_torque.get(aid, 0.0) != 0.0:
                        self._send_set_input_torque(aid, 0.0)
                        self.last_command_torque[aid] = 0.0
                        self.last_command_time[aid] = now
                elif self.last_command_vel.get(aid, 0.0) != 0.0:
    ...
    def _stop_all_axes(self):
        ...
        for axis_id in self.axis_ids:
            ...
            self._send_set_input_torque(axis_id, 0.0)
